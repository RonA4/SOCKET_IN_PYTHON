...
if __name__ == "__main__":
    ...
    The code we added allows users to evaluate mathematical expressions by choosing from predefined options or entering custom input.
    It communicates with a server using the 'client' function.

    1. Define the predefined expressions (options 1-6).
    2. The user will enter his choice
    3. Option to include step by step results and responses from the server or the proxy.
    4. The user will enter "0" in order to exit the program, that is, to disconnect from the server or the proxy.

    """

    print("Choose an option to choose a calculation:")
    print("1. max(2, 3) + 3")
    ...
    print("6. 4 * (3 + 5)")
    print("0. Disconnect from the proxy/server")
    user_choice = input("Enter choice or expression: ").strip()

    while user_choice != "0":
        try:
            if user_choice.isdigit() and int(user_choice) in range(1, 6):
                # Use predefined examples based on user input
                predefined_expressions = [
                    mul_b(
                        div_b(sin_f(max_f(2, mul_b(3, 4), 5, mul_b(6, div_b(mul_b(7, 8), 9)), div_b(10, 11))), 12),
                        13),
                    add_b(max_f(2, 3), 3),
                ...
                expr = predefined_expressions[int(user_choice) - 1]
            ...
            print(f"Error: Invalid expression input - {e}")
            exit(1)
