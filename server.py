    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        while True:
            try:
                ...
                # * Fill in start (2)
                #We are waiting for a connection to the server,
                # after a client has connected he gets his own specific socket and
                # the client's IP address is set in address
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            #receive data from the client ,as the max data packet is BUFFER_SIZE
            ...
            try:
                ...
                #send thr response back to the client
                #we sent it to the client and close the connection
